...
void genLA(int level, int offset) {
  emitLA(codeBlock, makeIntWORD(level), makeIntWORD(offset));
}

void genLV(int level, int offset) {
  emitLV(codeBlock, makeIntWORD(level), makeIntWORD(offset));
}
...
void genCALL(int level, CodeAddress label) {
  emitCALL(codeBlock, makeIntWORD(level), makeIntWORD(label));
}
