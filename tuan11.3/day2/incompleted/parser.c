...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_FLOAT:
    eat(TK_FLOAT);
    constValue=makeFloatConstant(atof(currentToken->string));
    break;
  case TK_IDENT:
    eat(TK_IDENT);
...
void compileAssignSt(void) {
  ...
  if(expType->typeClass == TP_FLOAT){
 checkFloatType(varType);
  }
...
Type* compileExpression3(Type* argType1) {
  ...
  case SB_PLUS:
    ...
    checkFloatOrIntType(argType1);
    argType2 = compileTerm();
 printf("hello\n");
    checkFloatOrIntType(argType2);
  ...
  case SB_SLASH:
    ...
    checkIntType(argType1);
  	...
  	checkIntType(argType2);
  	  genMOD();
  	resultType = compileTerm2(argType2);
...
Type* compileFactor(void) {
  ...
  case TK_NUMBER:
    eat(TK_NUMBER);
    type = intType;
    genLC(makeIntWORD(currentToken->value));
    break;
  case TK_CHAR:
    eat(TK_CHAR);
    type = charType;
   ...
   case TK_FLOAT:
    eat(TK_FLOAT);
    type=makeFloatType();
    //printf("caafn sua cho day\n");
    genLC(makeFloatWORD(atof(currentToken->string)));
    break;
  case TK_IDENT:
    eat(TK_IDENT);
    ...
    case OBJ_CONSTANT:
      switch (obj->constAttrs->value->type) {
            case TP_INT:
              	type = intType;
                genLC(makeIntWORD(obj->constAttrs->value->intValue));
              	break;
            case TP_CHAR:
              	type = charType;
                genLC(makeIntWORD(obj->constAttrs->value->charValue));
              	break;
             case TP_FLOAT:
             	type=floatType;
			    //printf("caafn sua cho day\n");
			    genLC(makeFloatWORD(obj->constAttrs->value->floatValue));
			    //genLC(makeIntWORD(obj->constAttrs->value->charValue));
            default:
      	         break;
      }
