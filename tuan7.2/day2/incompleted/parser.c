    ...
    do {
      ...
      eat(SB_EQ);
        ...
        if(lookAhead->tokenType==TK_NUMBER){
            makeIntConstant(lookAhead->value);
        }
        else if(lookAhead->tokenType==TK_CHAR){
          obj->constAttrs->value = makeCharConstant(lookAhead->string[0]);
        }
	else if(lookAhead->tokenType==TK_FLOAT){
          obj->constAttrs->value = makeFloatConstant(atof(lookAhead->string));
        }
...
switch(lookAhead->tokenType ){
 ...
 case KW_BEGIN:
   break;
 default:
   error(ERR_INVALID_VARIABLE,lookAhead->lineNo,lookAhead->colNo);
 }
}
...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_NUMBER:
    ...
    break;
  case TK_FLOAT:
    constValue=makeFloatConstant(atof(lookAhead->string));
    eat(TK_FLOAT);
    break;
  case TK_IDENT:
...
case KW_FLOAT: 
  ...
  case KW_ARRAY:
    ...
    eat(SB_RSEL);
    eat(KW_OF);
    type=makeArrayType(number,compileType());
    break;
  case TK_IDENT:
    type=duplicateType(lookupObject(lookAhead->string)->typeAttrs->actualType);
    eat(TK_IDENT);
    break;
  default:
